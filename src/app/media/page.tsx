    ...
    <main className="relative z-10 pt-24">
      <section className="bg-void">
        ...
        {featured ? (
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group relative mx-auto block aspect-[16/8] max-w-6xl overflow-hidden border-y border-steel"
          >
            {featured.imagePath ? (
              <Image
            ...
            <div className="relative flex h-full flex-col justify-end bg-gradient-to-t from-void via-void/40 to-transparent px-6 py-8 md:px-10">
              <span className="mb-6 flex h-16 w-16 items-center justify-center border border-bone text-bone">
                ▶
              </span>
              <p className="font-display text-[10px] tracking-[0.24em] text-blood uppercase">
                {featured.kind} · {featured.length}
              </p>
              <h2 className="mt-2 font-display text-3xl tracking-[0.08em] text-bone uppercase md:text-5xl">
                {featured.title}
              </h2>
          ...
          <div className="group relative mx-auto aspect-[16/8] max-w-6xl overflow-hidden border-y border-steel">
            {featured.imagePath ? (
              <Image
                src={featured.imagePath}
                alt={featured.title}
      ...
      {rest.length ? (
        <section className="overflow-hidden border-b border-steel bg-obsidian py-10">
          ...
          <div className="flex gap-4 overflow-x-auto px-5 pb-4 md:px-8">
            {rest.map((video) => (
              <article key={video.slug} className="min-w-[260px] border border-steel bg-void">
                {video.imagePath ? (
                ...
                <div className="p-5">
                  ...
                  {video.embedUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-4 inline-block font-display text-[10px] tracking-[0.2em] text-blood uppercase"
                    >
                      Watch
                    </a>
                  ) : null}
