...
export default function CheckoutPage() {
  ...
  return (
    <main className="relative z-10 mx-auto max-w-3xl px-5 pt-28 pb-20 md:px-8">
      ...
      <h1 className="mt-3 font-display text-4xl tracking-[0.08em] text-bone uppercase">Checkout</h1>
      {items.length === 0 ? (
        ...
        <>
          <ul className="mt-10 divide-y divide-steel border border-steel">
            {items.map((item) => (
              <li key={item.id} className="flex items-center justify-between px-5 py-4">
                ...
                <p className="text-sm text-mist">{money(item.priceCents * item.qty)}</p>
              </li>
            ))}
          </ul>
          ...
          <button
            ...
            className="mt-8 border border-blood bg-blood px-8 py-3 font-display text-[11px] tracking-[0.22em] text-bone uppercase hover:bg-ember disabled:opacity-50"
          >
